...
drop table mdo_cell_week
go

create table mdo_cell_week(
	...
	V_SD_RT						float,
	V_LD_RT						float
	unique(WEEK_ID, ttml, noc, province, district, sitename, cell_node_id)
)
go


alter proc sp_mdo_cell_week
	@offset int = 0, 
	@limit int = 100, 
	@acceptPartial bit = 0,
	@project varchar(max) = 'DEFAULT',

	--@WEEK_ID_filter						int = null,
 ...
 --   @SITENAME_filter					varchar(250) = null,
	...
	@WEEK_ID_filter						varchar(max) = null,
...
as begin
	...
	--if @WEEK_ID_filter is not null
	--	set @filter = @filter + ' and WEEK_ID = '	+ CAST(@WEEK_ID_filter as varchar(max));
	...
	if @WEEK_ID_filter is not null
		set @filter = @filter + ' and WEEK_ID in(' + @WEEK_ID_filter + ')';
	if @TTML_filter is not null
...
GROUP BY
	...
	MERGE mdo_cell_week AS Target
	USING (
		select week_id, isnull(ttml, '') ttml, isnull(noc, '') noc, isnull(province, '') province, isnull(district, '') district, 
	...
	WHEN NOT MATCHED BY TARGET THEN
		insert (
			WEEK_ID, TTML, NOC, PROVINCE, DISTRICT, SITENAME, CELL_NODE_ID, 
			WEB_TOTAL, MDO_VIDEO_COUNT, WEB_RESP_SUCC, WEB_AVG_RESP_DELAY, WEB_AVG_DISPLAY_DELAY, WEB_DL_TIME, WEB_DL_DATA, WEB_BROWS_SUCC, 
			WEB_BROWS_TOTAL, VIDEO_STREAM_SUCC, VIDEO_STREAM_TOTAL, VIDEO_STREAM_START_DELAY, VIDEO_STREAM_STALL, VIDEO_STREAM_DATA, VIDEO_STREAM_TIME, 
			VIDEO_HD_NUM, VIDEO_SD_NUM, VIDEO_LD_NUM, HTTPS_WEB_DL_DATA, HTTPS_WEB_DL_TIME, HTTPS_VIDEO_STREAM_DATA, HTTPS_VIDEO_STREAM_TIME, 
		...
		values (
			Source.WEEK_ID, Source.TTML, Source.NOC, Source.PROVINCE, Source.DISTRICT, Source.SITENAME, Source.CELL_NODE_ID, Source.WEB_TOTAL, 
...
exec sp_mdo_cell_week 0, 7, 1, 'noc_mdo', default, default, default, default, default, default, default 
exec sp_mdo_cell_week 0, 7, 1, 'noc_mdo', default, default, default, default, 'dsdsdsdsds', 'DNHC05', 1382001 
exec sp_mdo_cell_week 0, 7, 1, 'noc_mdo', default, default, default, default, 'Phu Cat', 'BDPC17', 123036 
exec sp_mdo_cell_week 0, 7, 1, 'noc_mdo', default, default, default, default, null, 'BDPC17', 123036 


select * from mdo_cell_week
