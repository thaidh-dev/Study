...
public class QuanLyNhanVien extends JPanel{
    ...
    public void initComponents() {
        ...
        lblAddress.setFont(new Font("Segoe UI", Font.ROMAN_BASELINE, 12));
        lblRole.setFont(new Font("Segoe UI", Font.ROMAN_BASELINE, 12));
        ...
        txaAddress.setFont(new Font("Segoe UI", Font.ROMAN_BASELINE, 12));
        txtUsername.setFont(new Font("Segoe UI", Font.ROMAN_BASELINE, 12));

        txtSearchEmployee.setPreferredSize(new Dimension(250, 25));
        txtSearchDepartment.setPreferredSize(new Dimension(250, 25));
    }

    public void addControls() {
        // LEFT
        JPanel pnSearchDepartment = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 5));
...
//        pnSearchEmployee.add(btnSearchEmployee);
    ...
    public void saveSuccess() {
        ...
        if (flagSave == CoreConstant.FLAG_INSERT && nvSelected.isVaitro() == CoreConstant.ADMIN) {
            //send mail thông báo đăng ký thành công
            new Thread(new Runnable() {
                @Override
                public void run() {
                    String to = nvSelected.getTaikhoan();
                    String sub = "Thông báo v.v tài khoản đăng nhập Phần mềm Quản Lý Tài Sản";
                    StringBuilder content = new StringBuilder("Chào bạn! \n");
                    content.append("Chúng tôi xin được thông báo: \n");
                    content.append("Mật khẩu đăng nhập Phần mềm Quản lý Tài Sản của bạn là: 123@123a. \n");
                    content.append("Bạn có thể đăng nhập vào phần mềm để đổi mật khẩu.");
                    MailUtils.sendMail(to, sub, content.toString());
                }
            }).start();
        }
