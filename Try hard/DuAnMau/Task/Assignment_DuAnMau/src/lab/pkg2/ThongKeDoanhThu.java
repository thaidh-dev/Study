...
public class ThongKeDoanhThu extends Khung {
    ...
    ButtonGroup bgr;
    JRadioButton rdoSLTang, rdoSLGiam, rdoDTGiam, rdoDTTang;
    String tenCot[] = {"Năm", "Số lượng học viên", "Doanh thu"};
    JButton btnKetXuat;
    List<Double> lstMoney = new ArrayList<>();
    ...
    public void taoDoiTuong() {
        ...
        lblTitle = new JLabel("Bảng thống kê doanh thu, số lượng người học từng chuyên đề theo năm");
        ...
        bgr = new ButtonGroup();
        rdoDTGiam = new JRadioButton("Doanh thu giảm dần");
        rdoDTTang = new JRadioButton("Doanh thu tăng dần");
    ...
    public void add() {
        ...
        JComponent a[] = {rdoDTTang, rdoSLTang, rdoDTGiam, rdoSLGiam};
        addChung(pnlInfoRight, gbc, a, 2, 2, new Insets(5, 5, 5, 5), 17);
    ...
    public void event() {
        ...
        rdoDTGiam.addItemListener(new ItemListener() {
            @Override
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.SELECTED) {
                    fillToTable();
        ...
        rdoDTTang.addItemListener(new ItemListener() {
            @Override
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.SELECTED) {
                    fillToTable();
        ...
        btnKetXuat.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                XSSFSheet sheet = workbook.createSheet("Thống kê doanh thu, số lượng người học từng chuyên đề theo năm");
                ...
                c0r1.setCellValue("Bảng thống kê doanh thu, số lượng người học từng chuyên đề theo năm");
                ...
                for (int i = 0; i < tblGridView.getRowCount(); i++) {
                    ...
                    for (int j = 0; j < 3; j++) {
                        ...
                        if (value.getClass() == Integer.class) {
                            ...
                            cellNormal.setCellValue(lstMoney.get(i));
                            
                            CellStyle cellStyleNormal2 = workbook.createCellStyle();
                            cellStyleNormal2.setFont(fontNormal);
                            cellStyleNormal2.setAlignment(HorizontalAlignment.CENTER);
                            cellStyleNormal2.setBorderRight(BorderStyle.MEDIUM);
                            cellStyleNormal2.setRightBorderColor(IndexedColors.BLACK.getIndex());
                            cellStyleNormal2.setBorderBottom(BorderStyle.MEDIUM);
                            cellStyleNormal2.setBottomBorderColor(IndexedColors.BLACK.getIndex());
                            cellStyleNormal2.setDataFormat((short) 7);

                            CellStyle cellStyleNormal3 = workbook.createCellStyle();
                            cellStyleNormal3.setFont(fontNormal);
                            cellStyleNormal3.setAlignment(HorizontalAlignment.CENTER);
                            cellStyleNormal3.setBorderRight(BorderStyle.MEDIUM);
                            cellStyleNormal3.setRightBorderColor(IndexedColors.BLACK.getIndex());
                            cellStyleNormal3.setDataFormat((short) 7);

                            if (i == tblGridView.getRowCount()-1) {
                                cellNormal.setCellStyle(cellStyleNormal2);
                                
                            }
                            else {
                                cellNormal.setCellStyle(cellStyleNormal3);
                            }
                        }
                    }
                }
    ...
    public void fillToTable() {
        String sql = "select * from dbo.fn_2(?)";
        
        if (rdoDTGiam.isSelected()) {
            sql = "select * from dbo.fn_2(?) order by 'money' desc";
        }
        else if (rdoDTTang.isSelected()) {
            sql = "select * from dbo.fn_2(?) order by 'money' asc";
        }
        else if (rdoSLGiam.isSelected()) {
            sql = "select * from dbo.fn_2(?) order by 'sl' desc";
        }
        else if (rdoSLTang.isSelected()) {
            sql = "select * from dbo.fn_2(?) order by 'sl' asc";
        }
        ...
        try {
            ...
            model.setRowCount(0);
            while (rs1.next()) {
                Object row[] = {rs1.getInt(1), rs1.getInt(2), rs1.getString(3)};
                model.addRow(row);
                lstMoney.add(rs1.getDouble(4));
            }          
        }
        catch (Exception e) {
        }        
