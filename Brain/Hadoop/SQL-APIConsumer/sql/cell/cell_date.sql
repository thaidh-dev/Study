...
alter proc sp_mdo_cell_date
	@offset int = 0, 
	@limit int = 100, 
	@acceptPartial bit = 0,
	@project varchar(max) = 'DEFAULT',

	--@DATE_ID_filter						int = null,
 ...
 --   @SITENAME_filter					varchar(250) = null,
	...
	@DATE_ID_filter						varchar(max) = null,
...
as begin
	...
	--if @DATE_ID_filter is not null
	--	set @filter = @filter + ' and DATE_ID = '	+ CAST(@DATE_ID_filter as varchar(max));
	...
	if @DATE_ID_filter is not null
		set @filter = @filter + ' and DATE_ID in(' + @DATE_ID_filter + ')';
	if @TTML_filter is not null
...
GROUP BY
	...
	WHEN NOT MATCHED BY TARGET THEN
		insert (
			DATE_ID, TTML, NOC, PROVINCE, DISTRICT, SITENAME, CELL_NODE_ID, 
			WEB_TOTAL, MDO_VIDEO_COUNT, WEB_RESP_SUCC, WEB_AVG_RESP_DELAY, WEB_AVG_DISPLAY_DELAY, WEB_DL_TIME, WEB_DL_DATA, WEB_BROWS_SUCC, 
			WEB_BROWS_TOTAL, VIDEO_STREAM_SUCC, VIDEO_STREAM_TOTAL, VIDEO_STREAM_START_DELAY, VIDEO_STREAM_STALL, VIDEO_STREAM_DATA, VIDEO_STREAM_TIME, 
			VIDEO_HD_NUM, VIDEO_SD_NUM, VIDEO_LD_NUM, HTTPS_WEB_DL_DATA, HTTPS_WEB_DL_TIME, HTTPS_VIDEO_STREAM_DATA, HTTPS_VIDEO_STREAM_TIME, 
		...
		values (
			Source.DATE_ID, Source.TTML, Source.NOC, Source.PROVINCE, Source.DISTRICT, Source.SITENAME, Source.CELL_NODE_ID, Source.WEB_TOTAL, 
...
drop table mdo_cell_date
go

create table mdo_cell_date(
	...
	V_SD_RT						float,
	V_LD_RT						float
	unique(date_id, ttml, noc, province, district, sitename, cell_node_id)
)
go
...
exec sp_mdo_cell_date 0, 7, 1, 'noc_mdo', default, default, default, default, default, default, default 

exec sp_mdo_cell_date 0, 7, 1, 'noc_mdo', default, default, default, default, 'Hai Chausdsdsdsds', 'DNHC05', 1382001 
exec sp_mdo_cell_date 0, 7, 1, 'noc_mdo', default, default, default, default, 'Ngoc Hoi, An Khe', 'KTNH13, GLAK23', '1378079, 192961' 
exec sp_mdo_cell_date 0, 7, 1, 'noc_mdo', default, default, default, default, null, 'DNHC05, DLKP14', '1382001, 1434124' 




select * from mdo_cell_date 
where district in ('Nha Trang', 'An Khe') and sitename in('KHNT3O', 'GLAK23') and CELL_NODE_ID in (null, 192961)



declare @a varchar(max) = 'abc, bcd, xxx'
select '''' + replace(@a, ', ', ''', ''') + ''''
